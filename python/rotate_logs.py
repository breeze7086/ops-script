            ...
            if string.split(fullpath_file, '.')[-1] == 'log':
                ...
                if file_date:
                    if int(current_date[-4:-2]) - int(file_date[-4:-2]) == 1:
                        if MONTH_DEADLINE[int(current_month)] - int(file_date[-2:]) + int(current_date[-2:]) > ROTATE_DAYS:
                            newest_file = get_dir_newest_file(fullpath_file)
...
def get_dir_newest_file(fullpath_filename):
    newest_file_time = 0
